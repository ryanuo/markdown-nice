...
function handlePressHotkey(type, content) {
...
function bindHotkeys(content, dialog) {
...
export function betterTab(cm) {
  if (cm.somethingSelected()) {
    cm.indentSelection('add')
  }
  else {
    cm.replaceSelection(
      cm.getOption('indentWithTabs') ? '\t' : new Array(cm.getOption('indentUnit') + 1).join(' '),
      'end',
      '+input',
    )
  }
