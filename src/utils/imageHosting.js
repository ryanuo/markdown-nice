...
// 七牛云对象存储上传
export async function qiniuOSSUpload({
  file = {},
  onSuccess = () => {},
  onError = () => {},
  onProgress = () => {},
...
// 用户自定义的图床上传
export async function customImageUpload({
...
// 阿里对象存储，上传部分
function aliOSSPutObject({ config, file, buffer, onSuccess, onError, images, content }) {
  let client
  try {
...
// 自动检测上传配置，进行上传
export function uploadAdaptor(...args) {
